...
% variables
Ik2 = -2;
num2 = ((Ik2)^2 + be1*abs(Ik2))*sign(Ik2);
Fe_m2A;
y2 = num2./Fe_m2A;    
z_m2A;
dx2 = [0:0.001:0.04];
...
plot(z_m2A,Fe_m2A,'k*');
plot(dx,num2./gx2);
title('Comparaison -1A et -2A')
xlabel('Position en z de la plaque (m)');
ylabel('Force électomagnétique (N)');
grid on;
legend('-1A experimental','approximation -1A','-2A experimental','Approximation -2A');
